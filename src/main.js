...
const selectAnswerForm = Devvit.createForm({
    ...
    try {
        ...
        try {
            const autoRemovalMode = await settings.get('autoRemovalMode');
            ...
            if (mode === 'on_solved') {
                ...
                console.log(`Post ${postId} scheduled for removal at ${removeAt.toISOString()}`);
            }
        }
        catch (removalErr) {
...
Devvit.addSchedulerJob({
    ...
    onRun: async (event, context) => {
        ...
        const { postId, subredditName } = event.data;
        try {
